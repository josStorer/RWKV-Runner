...
export const readRoot = async (port?: number) => {
  port = port || commonStore.getCurrentModelConfig().apiParameters.apiPort
  return fetch(`http://127.0.0.1:${port}`)
}

export const exit = async (timeout?: number, port?: number) => {
...
export const switchModel = async (body: any, port?: number) => {
  port = port || commonStore.getCurrentModelConfig().apiParameters.apiPort
