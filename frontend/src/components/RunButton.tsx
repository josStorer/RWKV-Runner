...
export const RunButton: FC<{
  ...
  const onClickMainButton = async () => {
    ...
    if (!webgpu && !webgpuPython) {
      if (['.st', '.safetensors'].some((ext) => modelPath.endsWith(ext))) {
        toast(t('Please change Strategy to WebGPU to use safetensors format'), {
          type: 'error',
        })
        commonStore.setStatus({ status: ModelStatus.Offline })
        return
    ...
    console.log(commonStore.settings.customPythonPath)
        ...
        .then(async (r) => {
          if (r.ok && !loading) {
            ...
            switchModel(
              ...
              .catch((e) => {
                commonStore.setStatus({ status: ModelStatus.Offline })
                toast(t('Failed to switch model') + ' - ' + (e.message || e), {
                  type: 'error',
                })
              })
              .finally(() => {
